...
def read_Annual_outputs_header(annual_outputs_filename):
    annual_outputs_file = open(annual_outputs_filename,"r")
    header = annual_outputs_file.readlines()[0].rstrip().split(",")
    annual_outputs_file.close()

    i_year_annualoutput = header.index('Year')
    i_prev_annualoutput = header.index('Prevalence')
    i_inc_annualoutput = header.index('Incidence')

    return [i_year_annualoutput,i_prev_annualoutput,i_inc_annualoutput]
...
def read_manicaland_files_and_make_summary_file(manicalandfiledir,outfilename,file_tag):
    ...
    AnnualOutputsfiles = glob.glob(manicalandfiledir + "/Annual_outputs_CL*"+file_tag)
    ...
    [i_year_annualoutput,i_prev_annualoutput,i_inc_annualoutput] = read_Annual_outputs_header(AnnualOutputsfiles[0])
    ...
    # These come from Annual_outputs files:
    outputs_incidence_overall_Annual_output = {}
    outputs_prevalence_overall_Annual_output = {}
    ...
    # Only pull out the years we need to match the MIHPSA_output files:
    years_needed_from_Annual_outputs = outputs_prevalence_M15_49.keys()
    # Process each of the Annual_outputs files:
    for n,f in enumerate(AnnualOutputsfiles):
        ...
        for i,line in enumerate(run_data):
            ...
            year = int(splitline[i_year_annualoutput])
            if(year in years_needed_from_Annual_outputs):
                if(n==0):
                    outputs_incidence_overall_Annual_output[year] = [float(splitline[i_inc_annualoutput])]
                    outputs_prevalence_overall_Annual_output[year] = [float(splitline[i_prev_annualoutput])]
                else:
                    outputs_incidence_overall_Annual_output[year] += [float(splitline[i_inc_annualoutput])]
                    outputs_prevalence_overall_Annual_output[year] += [float(splitline[i_prev_annualoutput])]
    ...
    outputs_to_store = {"Npop15_49":outputs_Npop15_49,
                        ...
                        # These two come from Annual_outputs:
                        "IncidenceAdult":outputs_incidence_overall_Annual_output,
                        "PrevalenceAdult":outputs_prevalence_overall_Annual_output
    ...
    list_of_outputs = ["PrevalenceM15_49","PrevalenceF15_49","PrevalenceAll15_49","ARTcoverage_M_15plus","ARTcoverage_F_15plus","N_ART_M_15plus","N_ART_F_15plus","prop_circ_15_49","N_VMMC_annual_15_49",
                       ...
                       "IncidenceM15_29","IncidenceF15_24","IncidenceM30_54","IncidenceF25_54", "IncidenceAdult", "PrevalenceAdult",
