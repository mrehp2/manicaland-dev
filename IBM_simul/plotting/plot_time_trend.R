...
i.prop.m.circ <- 22
i.m.vmmc <- 23
i.n.womenwaitingPrEP <- 24
i.n.womenonPrEP.adherent <- 25
i.n.womenonPrEP.semiadherent <- 26
i.n.menonPrEP.adherent <- 27
i.n.menonPrEP.semiadherent <- 28
i.n.mtct.transmissions <- 29
i.n.mtct.deaths <- 30
i.n.mtct.alive.agefourteen <- 31
i.frac.mtct.alive.onART.agefourteen <- 32
i.ndied <- 33
i.frac.diedfromhiv <- 34
i.n.hsv2.m <- 35
i.n.hsv2.f <- 36
i.incidence.hsv2 <- 37
i.n.hivtests <- 38
i.frac.acute.infections <- 39
...
annualoutput.filenames <- extract.annualoutput.filenames(fit.data,"/home/mike/MANICALAND/manicaland-dev/IBM_simul/results_2021_05_11/")
...
pdf("Plot_prevalence_prevention.pdf")
par(mar = c(5, 5, 3, 5))
plot(data.run1[[i.year]],100*data.run1[[i.prevalence]],type="l",xlim=c(1980,2040),xlab="",ylab="Overall HIV prevalence (%)")

par(new=TRUE)
plot(data.run1[[i.year]],100*data.run1[[i.prop.m.circ]],type="l",lty=2,col="blue",xlim=c(1980,2040),xlab="",ylab="",xaxt="n",yaxt="n")
axis(side=4)
mtext("Uptake of prevention activities", side = 4, line = 3)
lines(data.run1[[i.year]],100*data.run1[[i.n.womenonPrEP.adherent]]/data.run1[[i.npopf]],type="l",col="red",lty=2)
legend("topleft",legend=c("HIV prevalence (%)","% of men circumcised","% of women on PrEP"),col=c("black","blue","red"),lty=c(1,2,2))
dev.off()
