...
int carry_out_processes(int t0, fitting_data_struct *fitting_data, patch_struct *patch,
    ...
    */
    ...
    /* This function needs to sit here for now.
       It loads the MIHPSA-specific parameters. Right now these are all constant, but I migth want to draw them from a range - in which case I need this to only be called from the current time (so that the pre-2023 part of the simulation remains the same as I don't call the additional gsl_rng until 2023).
    */
    if(MIHPSA_MODULE==1){
	if(t0==2023){
	    printf("Calling fix_MIHPSA_parameters now\n");
	    for(p = 0; p < NPATCHES; p++)
		fix_MIHPSA_parameters(patch, p);
	}
    }
	...
	if(patch[0].country_setting==ZIMBABWE){
	    ...
	    if(MIHPSA_MODULE==1){
		if(patch[0].param->MIHPSA_params.FLAGS.remove_CUPP==1){
		    /* Removal of CUPP activities increases multiple partners. */
		    mihpsa_minimalscenario_increasemulitplepartners(t, patch);
		}
	    }
		...
		for(p = 0; p < NPATCHES; p++)
			...
			if(patch[p].param->MIHPSA_params.FLAGS.remove_CUPP==1){
			    if(t==2023){
				//fix_MIHPSA_parameters(patch, p);
				/* The "0.8" and "0.9" should match what is used in update_condomrates_MIHPSA() in prevention_cascades.c. */
				MIHPSA_sweep_through_condomusepartnerships_reduce_forcondomuse_minimalscenario(patch, p, t);
			    }
			}
...
int carry_out_processes_by_patch_by_time_step(int t_step, int t0, fitting_data_struct *fitting_data,
    ...
    */
    ...
    /* if(i_run==1 && p==0 && ((t0+t_step/48.0)>2046)){ */
    /* 	printf("PrEP status for individual 227641 = %i next PrEP_event=%i\n",patch[p].individual_population[227641].PrEP_cascade_status,patch[p].individual_population[227641].next_PrEP_event); */
    /* } */
    ...
    if(t >= patch[p].param->COUNTRY_HIV_TEST_START && t < (patch[p].param->COUNTRY_HIV_TEST_START + TIME_STEP) && DO_HIV_TESTING == 1){
		...
		draw_hiv_tests(patch[p].param, patch[p].age_list, t0, patch[p].cascade_events, patch[p].n_cascade_events, patch[p].size_cascade_events, patch[p].country_setting, patch, p);
        }
    }
    ...
    if((HIVTESTSCHEDULE == 1) && (t0 >= 2006) && (t_step == 0) && DO_HIV_TESTING == 1){
        ...
        draw_hiv_tests(patch[p].param, patch[p].age_list, t0, patch[p].cascade_events,
            patch[p].n_cascade_events, patch[p].size_cascade_events, patch[p].country_setting, patch, p);
    }
...
(t_step==patch[p].param->start_time_hiv_discretised_timestep)
	...
	if(t >= patch[p].param->COUNTRY_VMMC_START){
	    /* if MIHPSA_SCENARIO_FLAG is a non-VMMC one, then this is done inside sweep_pop_for_VMMC_per_timestep_given_barriers(): */
