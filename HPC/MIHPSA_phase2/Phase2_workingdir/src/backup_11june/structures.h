...
struct individual{
    ...
    int reason_next_HIV_test; /* Allows us to distinguish HIV tests that occur because of PrEP (and potentially other services). */
...
/* This structure contains arrays to sample those who get a MIHPSA intervention (currently PrEP). */
typedef struct{
    /* Stores for the people who will be on PrEP
       We create two stores - one is those who are the eligible group (e.g. all individuals with non-regular partners)
       If there aren't enough of these, then we create a second pool of other people at highish risk (e.g. all those with multiple partners who don't have a non-regular partner): */
    individual *potential_prep_initiates_pointers_primarypop[MIHPSA_INTERVENTION_SAMPLE_MAX_ARRAY_SIZE];
    individual *potential_prep_initiates_pointers_othereligible[MIHPSA_INTERVENTION_SAMPLE_MAX_ARRAY_SIZE];    
    individual *new_prep_initiates_pointers_primarypop[MIHPSA_INTERVENTION_SAMPLE_MAX_ARRAY_SIZE];
    individual *new_prep_initiates_pointers_othereligibles[MIHPSA_INTERVENTION_SAMPLE_MAX_ARRAY_SIZE];


}MIHPSA_intervention_sample_struct;

/* Structure contains flags for each bit of MIHPSA (e.g. minimal scenario = remove SBCC and CUPP). */
typedef struct{
    /* These are removed under minimal scenario (see Dropbox/projects/MIHPSA_Zimabwe2021/Phase2/Outputs_Dec2023/PopART-IBM_modelling_minimal_scenario.pptx): */
    /* Removing SBCC reduces in HIV testing in AGYW 14-24: */
    int remove_SBCC;
    int remove_CUPP;

    /* Under "essential" scenario, oral PrEP coverage is fixed to be 37,144 AGYW 15-24 at risk (FSW or non-regular partner) from 2023 onwards.
       For "minimal" scenario(s) there is no oral PrEP from 2023 onwards. */
    int MIHPSA_oralPrEP_from2023;

    /* Under minimal/essential scenarios, VMMC is switched off from 2023 onwards. */
    int remove_VMMC_from2023;

    /* For "minimal" scenario(s) there is only testing in ANC & PD (women) and testing in symptomatic people from 2023 onwards. */
    int only_ANC_PD_symptomatic_testing_from2023;
    
}MIHPSA_intervention_flags;
...
/* Structure contains parameters specific to MIHPSA: */
typedef struct{
    /* CUPP = Condom use promotion and provision. In minimal scenario we remove the effects of CUPP. 
       SBCC = social behaviour chaneg communication. Also removed in minimal scenario. 
       See Dropbox\projects\MIHPSA_Zimabwe2021\Phase2\MIHPSA Zimbabwe modellers call 29Nov2023_withNotes.pptx slide 2. */

    MIHPSA_intervention_flags FLAGS;
    
    double increase_in_npartners_without_CUPP;
    double decreasein_condomnonuse_dueto_CUPPon_LT;
    double decreasein_condomnonuse_dueto_CUPPon_nonreg;
    double change_in_AGYW14to24_HIVtesting_dueto_SBCC;
    //long change_in_npartners_dueto_CUPP_UL;
...
typedef struct {
    ...
    /* MIHPSA project parameters: */
    MIHPSA_param_struct MIHPSA_params;
    ...
    /* Allows us to store the scale factor to make the simulated population the same as the country-level population. */
    double mihpsa_population_scale;
...
typedef struct{
    long N_newbirths; /* Number of children born. */
    long N_newbirths_HIVpos; /* Number of children born who are HIV+. */
    long N_deaths_15plus[N_GENDER];
    ...
    /* Emergency ART channel: */
    long N_HIVtests_symptomatic_15plus[N_GENDER];
    long N_HIVtests_15to24F;
    long N_newHIVinfections_15plus[N_GENDER];
    long N_newHIVinfections_15to24[N_GENDER];
    long N_newHIVinfections_25to49[N_GENDER];
    long N_newHIVdiagnoses_15plus[N_GENDER];
    long N_newHIVdiagnoses_15to24F;
    long N_VMMC_15to49;
    ...
    /* Number of ART reinitiations (ie of someone previously on ART). */
    long N_ARTreinitiations_15plus;
    /* future YLL (assuming 80 years life expectancy), all incurred at the calendar year of death. Undiscounted and  with years of life remaining discounted by 3% per year. */
    double YLL_15plus[N_GENDER];
    double YLL_children_under15;
    double YLL_15plus_discounted[N_GENDER];
    double YLL_children_under15_discounted;
...
typedef struct{
    ...
    MIHPSA_intervention_sample_struct *MIHPSA_intervention_sample; 
