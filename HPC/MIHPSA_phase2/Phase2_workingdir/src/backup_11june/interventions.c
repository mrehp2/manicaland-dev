...
void carry_out_PrEP_events_per_timestep(double t, patch_struct *patch, int p, cumulative_outputs_struct *cumulative_outputs){
	...
	*** Call:
	remove_from_cascade_events(indiv, patch[p].cascade_events, patch[p].n_cascade_events, patch[p].size_cascade_events, double t, patch[p].param) in demographics.c

	if(HIV_TEST_WHEN_ON_PrEP==1){
	    If positive, don't start PrEP
	    If negative, check how long they're on PrEP for. If it is long enough, schedule a new HIV test.
            schedule_next_PrEP_hiv_test(indiv, param, int year, 
					patch[p].cascade_events, patch[p].n_cascade_events, patch[p].size_cascade_events, double x_time)
	}
