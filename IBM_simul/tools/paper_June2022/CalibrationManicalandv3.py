...
if len(sys.argv)>1:
    if (sys.argv[1]=="R7"):
        allrounds=[[7]]
    elif (sys.argv[1]=="R1-7"):
        allrounds = [[1,2,3,4,5,6,7]]
    elif (sys.argv[1]=="R67"):
        allrounds = [[6,7]]
else:
    allrounds = [[1,2,3,4,5,6,7],[6,7],[7]]
    
currentdir = os.getcwd().split("/")[-1]
if (currentdir[0:7]=="IMPACT_" or currentdir=="testing"):
    calibration_root_dir = "params/"
else:
    calibration_root_dir = "RESULTS/"

N_BEST_RUNS_TO_GET = 10

print "Calibrating for ",allrounds
...
def get_calibration_results_dirs(calibration_root_dir):
    ...
    if checked_dirs:
        ...
        possible_calibration_results_dirs.remove(calibration_root_dir)
        if (calibration_root_dir+"Output" in possible_calibration_results_dirs):
            checked_dirs = [calibration_root_dir+"Output"]
        print "Using ",possible_calibration_results_dirs
...
#def structure_ibm_data(model_data_array, header):
    ...
    nruns = len(model_data)   # read_calibration_file() removes the first line as it's a header.
...
#####HERE
            ...
            for r in rounds:      # Rounds of the given survey type:
                ...
                for t in survey_numerators:   # Outcomes (prevalence, 90-90-90 etc collected in the survey).
                    ...
                    if not(survey_data[s][r][t]=={ele:{} for ele in genders}):
                        for g in genders:
                            ## This is an alternative to the above (where I make sure the dictionary is non-empty) - make we have survey data for this outcome:
                            #if survey_data[s][r][t][g]:
    ...
    best_fits_data = {}
    #print "data_keys are",best_fits_likelihoods.keys()
    # best_fits_likelihoods.keys() are tuples e.g. ('1_0', 87)
...
for rounds in allrounds:
    ...
    tag = "".join([str(x) for x in rounds])
    best_model_calibration_filename = calibration_root_dir+"Calibration_data_bestfits"+tag+".csv"
    best_model_likelihood_filename = calibration_root_dir+"goodfits"+tag+".txt"
