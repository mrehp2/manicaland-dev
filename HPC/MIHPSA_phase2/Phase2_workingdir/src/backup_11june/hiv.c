 ...
 * schedule_generic_cascade_event(): given a time t_event and a person indiv, schedules an event in the array cascade_event[] for that person. Note that this function does not know what type of event it is, so that is set separately in the function calling schedule_generic_cascade_event(). Only called if HIVTESTSCHEDULE=0.
 * schedule_new_hiv_test(): function that sets up a HIV test in the future for someone. Called by draw_initial_hiv_tests() at the start of HIV testing in that country and by hiv_test_process() for people who just tested negative. Only called if HIVTESTSCHEDULE=0.
...
void draw_initial_SPVL(individual *seroconverter, parameters *param){
...
int draw_initial_cd4(parameters *param, int spvl_cat){
    double x = gsl_rng_uniform(rng);

    if(x < param->cumulative_p_initial_cd4_gt500[spvl_cat])
        return 0;
    else if(x < param->cumulative_p_initial_cd4_350_500[spvl_cat])
	return 1;
    else if(x < param->cumulative_p_initial_cd4_200_350[spvl_cat])
	return 2;
    else
        return 3;
}
...
void hiv_acquisition(individual* susceptible, double time_infect, patch_struct *patch, int p,
	...
	for (i_cond_partners=0; i_cond_partners<susceptible->n_partners; i_cond_partners++){
		...
		if (susceptible->cascade_barriers.use_condom_in_this_partnership[i_cond_partners]==USECOND)
		    PER_PARTNERSHIP_HAZARD_TEMPSTORE[i] *= (1.0-patch[p].param->eff_condom);
		
	    }
	    /* else{ */
	    /* 	printf("Error - mismatched partner pairs %li %li\n",susceptible->partner_pairs[i_cond_partners]->ptr[partner_gender]->id,temp_HIVpos_partner->id); */
	    /* } */
	    //        temp_HIVpos_partner = susceptible->partner_pairs_HIVpos[i]->ptr[partner_gender];
	...
	if(MIHPSA_MODULE==1 || EXTENDED_OUTPUTS_NEWINFECTIONS_YOUNG_OLD==1){
		...
		patch[p].cumulative_outputs->cumulative_outputs_MIHPSA->N_newHIVinfections_15plus[susceptible->gender]++;
	    }	
...
void new_infection(double time_infect, int SEEDEDINFECTION, individual* seroconverter, 
    ...
    if(SEEDEDINFECTION == TRUE)
        ...
        draw_initial_SPVL(seroconverter, param);
    else{
    ...
    seroconverter->cd4 = draw_initial_cd4(param,seroconverter->SPVL_cat);
...
void next_hiv_event(individual* indiv, individual ***hiv_pos_progression, 
    ...
    if(indiv->cd4 == 3){
	    ...
	    if(gsl_ran_bernoulli(rng, param->p_emergency_ART) == 1){
	        ...
	        cumulative_outputs->cumulative_outputs_MIHPSA->N_HIVtests_symptomatic_15plus[indiv->gender]++;
    ...
    for(n = 0; n < n_events; n++){
		...
		overall_partnerships->susceptible_in_hsv2serodiscordant_partnership,
            ...
            // If next event is CD4 progression
                ...
                update_ART_state_population_counters_cd4_change(t, patch[p].n_infected_by_all_strata, indiv->cd4, indiv->cd4+1, indiv->DoB, indiv->gender, indiv->sex_risk, indiv->ART_status);
		...
		/* Record the initiation, and if this is a reinitiation: */
		if(floor(t-indiv->DoB)>=15){
			...
			//printf("indiv->PANGEA_date_firstARTstart=%lf t=%lf\n",indiv->PANGEA_date_firstARTstart,t);
		    ...
		    patch[p].calendar_outputs, patch[p].n_infected_by_all_strata);
            }else{
...
void start_ART_process(individual* indiv, parameters *param, double t, 
    ...
    individual ***hiv_pos_progression, long *n_hiv_pos_progression, long *size_hiv_pos_progression,
		       int is_emergency, file_struct *file_data_store, calendar_outputs_struct *calendar_outputs, population_size_one_year_age_hiv_by_stage_treatment *n_infected_by_all_strata){
    ...
    update_ART_state_population_counters_ARTcascade_change(t, n_infected_by_all_strata, indiv->ART_status, EARLYART, indiv, FALSE);
...
void draw_hiv_tests(parameters *param, age_list_struct *age_list, int year, 
    ...
    probability_get_hiv_test_in_next_window(p_test, &t_gap, country_setting, year, 
					    param->COUNTRY_HIV_TEST_START, param, patch, p);
    ...
    for(g = 0; g < N_GENDER; g++){
		...
		p_test_indiv = p_test[g];


	    if(MIHPSA_MODULE==1){
		/* Under 'minimal' scenario, remove SBCC. SBCC reduces HIV testing in 14-24 year olds. */ 
		if(param->MIHPSA_params.FLAGS.remove_SBCC==1){
		    if(g==FEMALE){
			if(year>=2023){
			    if(year==2023 && aa==1)
				printf("Adjusting HIV testing rate for MIHPSA minimal scenario\n");
			    if(aa>=(14-AGE_ADULT) && aa<=(24-AGE_ADULT))
				p_test_indiv *= param->MIHPSA_params.change_in_AGYW14to24_HIVtesting_dueto_SBCC;
			    //p_test_indiv = 0;
	    ...
	    //printf("Year %i, Age adjustment for age %i=%lf p=%lf\n",year,aa+AGE_ADULT,param->p_HIV_background_testing_age_adjustment_factor[aa],p_test_indiv);
...
void probability_get_hiv_test_in_next_window(double *p_test, double *t_gap, int country_setting,
					     int year, int COUNTRY_HIV_TEST_START, parameters *param, patch_struct *patch, int p){
    /*
    ...
    */
    ...
    /* Currently the population is scaled to the 2021 WPP total (adult+child) population (see get_mihpsa_project_outputs_v3.py). 
       For MIHPSA status quo scenaro (19 April 2022) we want to scale the total number of HIV tests so that it is below 3m tests per year from 2021 onwards. So 
    */

    /* 2021 WPP total population is 8773997.323+6318618.0 = 15092615.323. */
    ...
    if(year == COUNTRY_HIV_TEST_START){
        ...
        *t_gap = 1;
	/* Makes sure that cannot have >3 million HIV tests per year: */ 
	if(MIHPSA_MODULE==1 && MIHPSA_CAP_HIV_TESTS==1){
	    if(year<2023 || param->MIHPSA_params.FLAGS.only_ANC_PD_symptomatic_testing_from2023==0){
		double RRtestMF = p_test[MALE]/p_test[FEMALE];
		p_test[FEMALE] = 0.37;
		//p_test[MALE] = 0.35;
		p_test[MALE] = RRtestMF * p_test[FEMALE];
		if(year==2021)
		    param->mihpsa_population_scale = count_15plus_scale(patch, p);
		else if(year>2021){
		    double pop_15plus = count_15plus_scale(patch, p);
		    double testing_adjustment = param->mihpsa_population_scale/pop_15plus;
		    p_test[FEMALE] *= testing_adjustment;
		    p_test[MALE] *= testing_adjustment;
		    printf("Scaled testing at t=%i M/F = %lf / %lf adjustment=%lf\n",year,p_test[MALE],p_test[FEMALE],testing_adjustment);
		
		}
	    }
	    /* Under certain scenarios (e.g. minimal) we switch off HIV testing except for ANC, PD (women) and symptomatic (emergency ART): */
	    else if(year>=2023 && param->MIHPSA_params.FLAGS.only_ANC_PD_symptomatic_testing_from2023==1){
		//double pop_15plus = count_15plus_scale(patch, p);
		//double testing_adjustment = param->mihpsa_population_scale/pop_15plus;
		/* The "0.37" is the hard-coded value of female testing from 2021 onwards (excluding testing adjustment): */
	    ...
	    /* Old code: 
	    double max_national_hiv_tests_per_year = 3000000;

	    if(year==2021){
		double WPP_total_pop_2021 = 15092615.323;
		param->mihpsa_population_scale = find_population_scale(patch, p, WPP_total_pop_2021);
		printf("Capping HIV tests at %lf per year with population scale %lf\n",max_national_hiv_tests_per_year,param->mihpsa_population_scale);
	    }
	    double N_m_temp, N_f_temp;
	    count_adult_pop_by_gender(patch, p, &N_m_temp, &N_f_temp);
	    printf("t=%i model M/F pops = %lf %lf COuntryPop=%lf Ntests=%lf\n",year,N_m_temp, N_f_temp,(N_m_temp+ N_f_temp)*param->mihpsa_population_scale, (param->mihpsa_population_scale * (p_test[MALE]*N_m_temp + p_test[FEMALE]*N_f_temp)));
	    if((param->mihpsa_population_scale * (p_test[MALE]*N_m_temp + p_test[FEMALE]*N_f_temp)) > max_national_hiv_tests_per_year){
		
		double RRtestMF = p_test[MALE]/p_test[FEMALE];
		
		p_test[FEMALE]= max_national_hiv_tests_per_year/(param->mihpsa_population_scale * (RRtestMF*N_m_temp + N_f_temp));
		p_test[MALE]= p_test[FEMALE]*RRtestMF;
		printf("Testing probabilties M/F before = %6.4lf %6.4lf, after = %6.4lf %6.4lf\n",param->p_HIV_background_testing_male_current,param->p_HIV_background_testing_female_current,p_test[MALE],p_test[FEMALE]);
	    }
	    */
	}
...
/* Function to generate a new HIV test (for someone who is on e.g. PrEP) at a fixed interval following a current one: */
void schedule_next_PrEP_hiv_test(individual* indiv, parameters *param, int year, 
    individual ***cascade_events, long *n_cascade_events, long *size_cascade_events, double x_time){
    /* For a given individual draw when they will next have an HIV test
    ...
    size_cascade_events : pointer to a long
    x_time : double
        Period of time (in years) when the next HIV test should occur (for oral PrEP this is 0.25 years).  
    ...
    */
    ...
    if(indiv->id == FOLLOW_INDIVIDUAL && indiv->patch_no == FOLLOW_PATCH){
	printf("At year=%i, new HIV test (due to PrEP or other service requiring routine HIV testing) for adult %ld scheduled at future time %6.4f.\n",
	       year,indiv->id, time_hiv_test);
	fflush(stdout);
    }
...
void hiv_test_process(individual* indiv, parameters *param, double t, individual ***cascade_events, 
	...
	if (floor(t-indiv->DoB)>=15){
	    ...
	    if((floor(t-indiv->DoB)<=24) && (indiv->gender)==FEMALE){
		patch[p].cumulative_outputs->cumulative_outputs_MIHPSA->N_HIVtests_15to24F += 1;
	    }
	}
    }
	...
	//if(age>=15 && age <=49) /* modified 29 June 2023. */
	if(age>=15){
    ...
    if (t>=param->COUNTRY_ART_START){       
	    ...
	    cumulative_outputs, calendar_outputs, patch[p].n_infected_by_all_strata);
        }
     ...
     *  either get a CD4 test in the future, or drop out: */
        ...
        if (!joins_preart_care(indiv,param,t,cumulative_outputs,calendar_outputs)){ /// SAME COMMENT AS BEFORE
            debug->art_vars[p].cascade_transitions[ARTNEG+1][CASCADEDROPOUT+1]++;
            dropout_process(indiv,param,t, cascade_events, n_cascade_events, size_cascade_events, hiv_pos_progression, n_hiv_pos_progression, size_hiv_pos_progression, cumulative_outputs,calendar_outputs, patch[p].n_infected_by_all_strata);
        }else{
...
void virally_suppressed_process(individual* indiv, parameters *param, double t, individual ***cascade_events, long *n_cascade_events, long *size_cascade_events,individual ***hiv_pos_progression, long *n_hiv_pos_progression, long *size_hiv_pos_progression, population_size_one_year_age_hiv_by_stage_treatment *n_infected_by_all_strata, int IS_NEW_ADULT){
    ...
    /* This tells us if the cd4 test is due to PopART (is_popart=1) or not (is_popart=0). Multiply by (1-IS_NEW_ADULT) to make sure that new adults who were infected via MTCT as children are not accidentally put as is_popart. */
    int is_popart = (indiv->next_cascade_event>=NCASCADEEVENTS_NONPOPART)*(1-IS_NEW_ADULT);
    ...
    update_ART_state_population_counters_ARTcascade_change(t, n_infected_by_all_strata, indiv->ART_status, LTART_VS, indiv, IS_NEW_ADULT);
	...
	/* When creating a new MTCT on ART we need to assign a value for PANGEA_t_next_cd4stage if they ever CD4 progress that is in the future from when the person is added to the simulation, as this value is used to set PANGEA_t_prev_cd4stage when calling next_hiv_event(). A good assumption is to take it to be the time at which they stop being VS. */
	if (IS_NEW_ADULT==TRUE)
	    indiv->PANGEA_t_next_cd4stage = time_end_vs;
	...
	/* Again, when creating a new MTCT on ART we need to assign a value for PANGEA_t_next_cd4stage if they ever CD4 progress that is in the future from when the person is added to the simulation, as this value is used to set PANGEA_t_prev_cd4stage when calling next_hiv_event(). A good assumption is to take it to be the time at which they stop being VS. */
	if (IS_NEW_ADULT==TRUE)
	    indiv->PANGEA_t_next_cd4stage = time_dropout;
...
void virally_unsuppressed_process(individual* indiv, parameters *param, double t, individual ***cascade_events, long *n_cascade_events, long *size_cascade_events, individual ***hiv_pos_progression, long *n_hiv_pos_progression, long *size_hiv_pos_progression, cumulative_outputs_struct *cumulative_outputs, calendar_outputs_struct *calendar_outputs, population_size_one_year_age_hiv_by_stage_treatment *n_infected_by_all_strata){
    ...
    update_ART_state_population_counters_ARTcascade_change(t, n_infected_by_all_strata, indiv->ART_status, LTART_VU, indiv, FALSE);
...
void dropout_process(individual* indiv, parameters *param, double t, individual ***cascade_events,
    ...
    */
    ...
    update_ART_state_population_counters_ARTcascade_change(t, n_infected_by_all_strata, indiv->ART_status, CASCADEDROPOUT, indiv, FALSE);
...
/* Update the counters in n_infected_by_all_strata for the number of HIV+ by one-year age group + gender
 ...
 We keep track of:
 - on EARLY ART
 - LT ART and virally suppressed 
 - LT ART but virally unsuppressed 
...
*/
void update_ART_state_population_counters_ARTcascade_change(double t, population_size_one_year_age_hiv_by_stage_treatment *n_infected_by_all_strata, int INITIAL_ART_STATE, int NEW_ART_STATE, individual *indiv, int IS_NEW_ADULT){


    /* For debugging: check transition is allowed (except for new adults): */
    if (IS_NEW_ADULT==FALSE)
	check_valid_ART_transition(INITIAL_ART_STATE,NEW_ART_STATE);
	...
	/* Update the 'old' ART state (if not a new adult): */
	if (IS_NEW_ADULT==FALSE)
	...
	/* Update the 'old' ART state counter for the oldest age group (note that oldest age group *cannot* be a new adult!): */
	(n_infected_by_all_strata->hiv_pop_size_oldest_age_gender_risk[indiv->gender][indiv->sex_risk][indiv->cd4][INITIAL_ART_STATE+1])--;
	 
	/* Update the 'new' ART state counter for the oldest age group: */
	(n_infected_by_all_strata->hiv_pop_size_oldest_age_gender_risk[indiv->gender][indiv->sex_risk][indiv->cd4][NEW_ART_STATE+1])++;
    }

}


/* Update n_infected_by_all_strata counter when progressing from one CD4 category to the next. */
void update_ART_state_population_counters_cd4_change(double t, population_size_one_year_age_hiv_by_stage_treatment *n_infected_by_all_strata, int INITIAL_CD4, int NEW_CD4, double DoB, int gender, int sex_risk, int iart){

    if (iart==ARTDROPOUT){
	printf("UNexpectedly using ARTDROPOUT!!!. Exiting\n");
	exit(1);
    }
    ...
    for (n=0; n<n_events; n++){
		...
		if(floor(t-indiv->DoB)>=15){
			...
			patch[p].cumulative_outputs->cumulative_outputs_MIHPSA->N_ARTreinitiations_15plus++;
                ...
                start_ART_process(indiv, patch[p].param, t, patch[p].cascade_events, patch[p].n_cascade_events, patch[p].size_cascade_events, patch[p].hiv_pos_progression, patch[p].n_hiv_pos_progression, patch[p].size_hiv_pos_progression,0, file_data_store, patch[p].calendar_outputs, patch[p].n_infected_by_all_strata);
            }
                ...
                virally_suppressed_process(indiv,patch[p].param,t, patch[p].cascade_events, patch[p].n_cascade_events, patch[p].size_cascade_events, patch[p].hiv_pos_progression, patch[p].n_hiv_pos_progression, patch[p].size_hiv_pos_progression,patch[p].n_infected_by_all_strata, FALSE);
            }
            ...
            //get_virally_unsuppressed(indiv,param,t, cascade_events, n_cascade_events, size_cascade_events);
                ...
                if (indiv->ART_status==LTART_VS)
                    ...
                    indiv->DEBUG_cumulative_time_on_ART_VU += (t-indiv->DEBUG_time_of_last_cascade_event);
                dropout_process(indiv,patch[p].param,t, patch[p].cascade_events, patch[p].n_cascade_events, patch[p].size_cascade_events, patch[p].hiv_pos_progression, patch[p].n_hiv_pos_progression, patch[p].size_hiv_pos_progression, patch[p].cumulative_outputs, patch[p].calendar_outputs, patch[p].n_infected_by_all_strata);
            }
            else{
