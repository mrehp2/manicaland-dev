...
void read_partnership_params(char *patch_tag, parameters *allrunparameters, int n_runs, int country_setting){
    ...
    */
	...
	if(country_setting==ZIMBABWE){
	    ...
	    for(ptype=0; ptype<N_PARTNER_TYPES; ptype++)
		calculate_current_c_within_patch(param_local, rr_m, rr_f, ptype);
	...
	for(ptype=0; ptype<N_PARTNER_TYPES; ptype++)
	    calculate_c_between_patches(param_local,ptype);
...
void read_cascade_params(char *patch_tag, parameters *allrunparameters, int n_runs){
    ...
    for (i_run = 0; i_run<n_runs; i_run++){
        ...
        checkreadok = fscanf(param_file,"%lg",&(param_local->p_HIV_background_testing_female_2010_2013));
        check_if_cannot_read_param(checkreadok,"param_local->p_HIV_background_testing_female_2010_2013");
        ...
        checkreadok = fscanf(param_file,"%lg",&(param_local->p_HIV_background_testing_male_2010_2013));
        check_if_cannot_read_param(checkreadok,"param_local->p_HIV_background_testing_male_2010_2013");
...
void read_cascade_barrier_params(char *patch_tag, parameters *allrunparameters, int n_runs){
    ...
    */
	...
	for (i_barrier_group=0; i_barrier_group<N_VMMC_PREVENTIONBARRIER_GROUPS; i_barrier_group++){	    
	    ...
	    //printf("i=%i after param_local->barrier_params.p_use_VMMC_present[i_barrier_group][0] = %lf\n",i_barrier_group,param_local->barrier_params.p_use_VMMC_present[i_barrier_group][0]);
	...
	for (i_barrier_group=0; i_barrier_group<N_PrEP_PREVENTIONBARRIER_GROUPS; i_barrier_group++){	    
	    checkreadok = fscanf(param_file,"%lg",&(param_local->barrier_params.p_use_PrEP_present[i_barrier_group][0]));
	...
	/* Read in the pre-intervention values - post-intervention is calculated afterwards with a multiplier. */
	for (i_barrier_group=0; i_barrier_group<N_VMMC_PREVENTIONBARRIER_GROUPS; i_barrier_group++){	    
	    checkreadok = fscanf(param_file,"%lg",&(param_local->barrier_params.p_use_VMMC_present[i_barrier_group][1]));
	    check_if_cannot_read_param(checkreadok,"param_local->barrier_params.p_use_VMMC_present[][1]");
	...
	for (i_barrier_group=0; i_barrier_group<N_COND_PREVENTIONBARRIER_GROUPS; i_barrier_group++){	    
	    checkreadok = fscanf(param_file,"%lg",&(param_local->barrier_params.p_use_cond_casual_present[i_barrier_group][1]));
	...
	/* Now populate param->barrier_params.p_use_cond_casual/LT, p_use_PrEP and p_use_VMMC for each run: */
	update_condomrates(param_local->start_time_simul, param_local);

	update_PrEPrates(param_local->start_time_simul, param_local);
