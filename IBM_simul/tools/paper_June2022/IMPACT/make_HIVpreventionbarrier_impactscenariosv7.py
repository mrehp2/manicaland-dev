...
# Factor to convert from the prevalence of VMMC (in R7) to an annual rate in 2019. Details of how I get this are in Dropbox\Manicaland\Model\VMMC\Model_VMMC_uptake_FINAL.xlsx, but basically I use the MIHPSA project DHIS-2 national VMMC data to get an annual rate of VMMC, and assume that the % of men who underwent VMMC in the given priority population is split according to the %s.
VMMC_amortization_factor = 0.20524131132744
...
##############
def read_cascade_csv_file(methods):
    cascade_param_file = ("/home/mike/Dropbox (SPH Imperial College)/Manicaland/Cascades/prevention_cascade.csv")
    cascade_file_ptr = open(cascade_param_file,"r")
    cascade_data_raw = cascade_file_ptr.readlines()
    cascade_file_ptr.close()

    cascade_data = {}

    for i in range(3,len(cascade_data_raw)):
        rowdata = cascade_data_raw[i].rstrip().replace('"','').split(",")
        tool = rowdata[0]
        if(tool in methods):
            s = rowdata[2]
            # Ignore the never sex group here:
            if (not(s=="Never had sex") and not(tool=="Male condoms" and not("Casual lst 2wks" in s))):
                gender = rowdata[1].rstrip().replace("Female","F").replace("Male","M")
                age_gp = s.lstrip().replace("Ever had sex ","").replace("Casual lst 2wks ","").replace(">=1 risk lst 12m ","").replace("-","_")
                pop =  gender + age_gp
                for i in [3,4,7,10]:
                    rowdata[i] = int(rowdata[i])
                #print "Pop=",pop,rowdata[3],rowdata[4],rowdata[7],rowdata[10]
                ...
                if(tool=="VMMC"):
                    # We split the amortization equally across the 3 parts of the cascade:
                    cascade_data[tool][pop] = [p_motivation*math.pow(VMMC_amortization_factor,1/3.0),p_access*math.pow(VMMC_amortization_factor,1/3.0),p_effuse*math.pow(VMMC_amortization_factor,1/3.0)]
                else:
                    cascade_data[tool][pop] = [p_motivation,p_access,p_effuse]
        else:
            print "Unknown tool ",tool
            sys.exit(1)
    print "Here is the cascade data",cascade_data
    return cascade_data
...
intervention_barrier_levels_allpops = {"Male condoms":[0.97, 0.9, 0.9],"PrEP":[0.3,0.9,0.9],"VMMC":[0.56,0.56,0.56]}

intervention_barrier_levels = {}

for tool in intervention_barrier_levels_allpops.keys():
    intervention_barrier_levels[tool] = {}
    for population in pops[tool]:
            intervention_barrier_levels[tool][population] = intervention_barrier_levels_allpops[tool][:]
...
#cascade_data = read_cascade_excel_file(prevention_methods) This is the old version.
cascade_data = read_cascade_csv_file(prevention_methods)


for tool in intervention_barrier_levels_allpops.keys():
    for population in pops[tool]:
        for i_cascade in range(3):
            if (intervention_barrier_levels[tool][population][i_cascade] < cascade_data[tool][population][i_cascade]):
                print "Modifying ",tool," pop=",population," i_cascade=",i_cascade
                intervention_barrier_levels[tool][population][i_cascade] = cascade_data[tool][population][i_cascade]
                
print "Here is the intervention data",intervention_barrier_levels
...
for tool in prevention_methods+["All_tools"]:
    ...
    for barriers in ["allbarriers","increase_motivation","increase_access","increase_effuse","remove_barriers","increase_motivation_access"]:
        ...
        for a_tool in tool_list:
            ...
            # These are the names of the column in the baseline param_processed_patch0_barriers.csv that we will modify (i_to_change is then the column number):
                ...
                if(barriers=="increase_motivation"):
                    new_param_values[a_tool][population] = intervention_barrier_levels[a_tool][population][0]*cascade_data[a_tool][population][1]*cascade_data[a_tool][population][2]
                elif(barriers=="increase_access"):
                    new_param_values[a_tool][population] = cascade_data[a_tool][population][0]*intervention_barrier_levels[a_tool][population][1]*cascade_data[a_tool][population][2]
                elif(barriers=="increase_effuse"):
                    new_param_values[a_tool][population] = cascade_data[a_tool][population][0]*cascade_data[a_tool][population][1]*intervention_barrier_levels[a_tool][population][2]
                elif(barriers=="increase_motivation_access"):
                    new_param_values[a_tool][population] = intervention_barrier_levels[a_tool][population][0]*intervention_barrier_levels[a_tool][population][1]*cascade_data[a_tool][population][2]
                elif(barriers=="remove_barriers"):
                    new_param_values[a_tool][population] = prod(intervention_barrier_levels[a_tool][population])
                else:
                ...
                #    new_param_values[a_tool][population] = new_param_values[a_tool][population]*VMMC_amortization_factor
...
for tool in prevention_methods:
    for population in pops[tool]:
        ...
        for barriers in ["allbarriers","increase_motivation","increase_access","increase_effuse","remove_barriers","increase_motivation_access"]:
            ...
            # Calculate the probability of the given population using the given tool for this "barrier" scenario:
            if(barriers=="increase_motivation"):
                new_param_value = intervention_barrier_levels[tool][population][0]*cascade_data[tool][population][1]*cascade_data[tool][population][2]
            elif(barriers=="increase_access"):
                new_param_value = cascade_data[tool][population][0]*intervention_barrier_levels[tool][population][1]*cascade_data[tool][population][2]
            elif(barriers=="increase_effuse"):
                new_param_value = cascade_data[tool][population][0]*cascade_data[tool][population][1]*intervention_barrier_levels[tool][population][2]


            elif(barriers=="increase_motivation_access"):
                new_param_value = intervention_barrier_levels[tool][population][0]*intervention_barrier_levels[tool][population][1]*cascade_data[tool][population][2]
            elif(barriers=="remove_barriers"):
                new_param_value = prod(intervention_barrier_levels[tool][population])
            else:
            ...
            #if(tool=="VMMC"):
                # Multiply by 0.20524131132744 to convert from a cumulative % circumcised to an annual %. The value is obtained in C:\Users\mpickles\Dropbox (SPH Imperial College)\Manicaland\Model\VMMC\Model_VMMC_uptake_FINAL.xlsx - it is the % of VMMC operations from 2013-2019 (from DHIS-2 data) that occurred in 2019. Assume that this same multiplier holds now.
            #    new_param_value = new_param_value*VMMC_amortization_factor
