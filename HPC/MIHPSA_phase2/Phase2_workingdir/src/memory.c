...
void blank_individual_array(individual *individual_population, int id_counter){
	...
	// These 3 are now pointers to an existing thing, so not needed. 
...
void alloc_patch_memoryv2(patch_struct *patch){
	...
	patch[p].n_infected_by_all_strata = malloc(sizeof(population_size_one_year_age_hiv_by_stage_treatment));
        if(patch[p].n_infected_by_all_strata==NULL)
        {
            printf("Unable to allocate n_infected_by_all_strata in alloc_all_memory. Execution aborted.");
            printf("LINE %d; FILE %s\n", __LINE__, __FILE__);
            fflush(stdout);
            exit(1);
        }
	...
	/* These is the arrays that store the  PrEP cascade events (independently of whether starts PrEP due to intervention or not): */
        patch[p].PrEP_events = malloc(MAX_N_YEARS*N_TIME_STEP_PER_YEAR*sizeof(individual**));  // PrEP_events[t] is a list of pointers to individuals whose next cascade event will happen at time step param.start_time_simul+t
	if(patch[p].PrEP_events==NULL){
            printf("Unable to allocate PrEP_events in alloc_all_memory. Execution aborted.");
            printf("LINE %d; FILE %s\n", __LINE__, __FILE__);
            fflush(stdout);
            exit(1);
        }
        ...
        patch[p].n_PrEP_events = malloc(MAX_N_YEARS*N_TIME_STEP_PER_YEAR*sizeof(long));
        if(patch[p].n_PrEP_events==NULL){
            printf("Unable to allocate n_PrEP_events in alloc_all_memory. Execution aborted.");
            printf("LINE %d; FILE %s\n", __LINE__, __FILE__);
            fflush(stdout);
            exit(1);
        }

        patch[p].size_PrEP_events = malloc(MAX_N_YEARS*N_TIME_STEP_PER_YEAR*sizeof(long));
        if(patch[p].size_PrEP_events==NULL){
            printf("Unable to allocate size_PrEP_events in alloc_all_memory. Execution aborted.");
            printf("LINE %d; FILE %s\n", __LINE__, __FILE__);
            fflush(stdout);
            exit(1);
        }
        ...
        patch[p].MIHPSA_intervention_sample = malloc(sizeof(MIHPSA_intervention_sample_struct));
        if(patch[p].MIHPSA_intervention_sample==NULL)
        {
            printf("Unable to allocate MIHPSA_intervention_sample in alloc_all_memory. Execution aborted.");
            printf("LINE %d; FILE %s\n", __LINE__, __FILE__);
            fflush(stdout);
            exit(1);
        }
			   ...
			   chips_sample_struct *chips_sample, cumulative_outputs_struct *cumulative_outputs, calendar_outputs_struct *calendar_outputs, long ****cross_sectional_distr_n_lifetime_partners, long ****cross_sectional_distr_n_partners_lastyear, PC_sample_struct *PC_sample, PC_cohort_struct *PC_cohort, PC_cohort_data_struct *PC_cohort_data, PrEP_background_sample_struct *PrEP_background_sample, PrEP_intervention_sample_struct *PrEP_intervention_sample, MIHPSA_intervention_sample_struct *MIHPSA_intervention_sample)
{
    ...
    /* MIHPSA intervention arrays: */
    free(MIHPSA_intervention_sample);
}
...
void free_patch_memory(patch_struct *patch){
    ...
    for (p=0; p<NPATCHES; p++){
		...
		patch[p].PC_sample, patch[p].PC_cohort, patch[p].PC_cohort_data,
	        ...
	        patch[p].MIHPSA_intervention_sample);
    }
    free(patch);
}
