...
paramsdir="PARAMS_COMMUNITY"+str(COMMUNITY)+"/"

paramsdir="GENERATED_PRIORS/PARAMS_COMMUNITY"+str(COMMUNITY)+"/"
codedir = "CODE/"
...
################################################################
######## Check params_times.txt:
################################################################
param_times_file = open(paramsdir+"param_times.txt","r")
params_times = param_times_file.read().split("\n")
param_times_file.close()

end_time_simul = int(find_line(params_times,"end_time_simul"," ",0,1))
check_error(end_time_simul,"end_time_simul",paramsdir+"params_times.txt")
if (end_time_simul!=2020):
    print "Set end_time_simul to 2020 ",paramsdir+"params_times.txt. Currently ",end_time_simul

NDHSROUNDS = int(find_line(params_times,"NDHSROUNDS"," ",0,1))
check_error(NDHSROUNDS,"NDHSROUNDS",paramsdir+"params_times.txt")
if (NDHSROUNDS!=NDHSROUNDS_BY_COUNTRY[COUNTRY]):
    print "Set NDHSROUNDS to",NDHSROUNDS_BY_COUNTRY[COUNTRY],", ",paramsdir+"params_times.txt. Currently ",NDHSROUNDS


################################################################
######## Check params_patchinfo.txt:
################################################################
param_patchinfo_file = open(paramsdir+"param_patchinfo.txt","r")
params_patchinfo = param_patchinfo_file.read().split("\n")
param_patchinfo_file.close()

community_id = find_line(params_patchinfo,"cluster_number"," ",0,1)
if (community_id==-1):
    print "Error - community_id not found in ",paramsdir+"params_patchinfo.txt"
    sys.exit(1)
if (community_id!=CLUSTERS[COMMUNITY]):
    print "Set community_id to ",CLUSTERS[COMMUNITY]," in",paramsdir+"params_patchinfo.txt. Currently ",community_id
...
def check_constants(codedir):
    ...
    CHECKCONSTANTS = [["PRINT_EACH_RUN_OUTPUT",0], # Note that this is the key constant as it controls most of the output.
                  ...
                  #["GENERATE_ANNUAL_PARTNERSHIPS_OUTPUTS",0],
                  ...
                  #["DEBUG_HIV_STATES",0],
                  #["DEBUG_ART_STATE",0],
                  #["DEBUG_CHIPS_STATES",0],
                  #["PRINT_EVERYTIMESTEP",0], # Generates Timestep_output csv file.
                  #["PRINT_PHYLOGENETICS_OUTPUT",0],
                  #["PRINT_PARTNERSHIP_NETWORK_SNAPSHOT",0],
                  #["PRINT_PARTNERS_OUTSIDE_COMMUNITY",0],
                  #["STORE_HAZARDS",0],
                  #["PRINT_PARTNERSHIPS_AT_PC0",0],
                  ["FOLLOW_INDIVIDUAL",-1],
                  ["MAX_POP_SIZE",320000],
                  ["RUN_PC",1]]
